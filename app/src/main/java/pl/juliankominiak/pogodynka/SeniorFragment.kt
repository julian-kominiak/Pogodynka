package pl.juliankominiak.pogodynka
...
class SeniorFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<Button>(R.id.switchSenior).apply {
            setOnClickListener {
                view.findNavController().navigate(R.id.action_seniorFragment_to_classicFragment)
            }
        }
    ...
    fun updateView(view: View, weatherData: JSONObject) {
        ...
        if (temperatureValue.toInt() < 15) {
            temperature.setTextColor(Color.BLUE)
        } else if (temperatureValue.toInt() in 15..25) {
        ...
        if (JSONObject(weatherData.getJSONObject("main").toString())
                .get("pressure").toString().toInt() in 1000..1015) {
            pressure.setTextColor(Color.GREEN)
        } else if (JSONObject(weatherData.getJSONObject("main").toString())
                .get("pressure").toString().toInt() > 1015) {
            pressure.setTextColor(Color.RED)
        } else {
            pressure.setTextColor(Color.BLUE)
        }
        ...
        sunrise.text = DateTimeFormatter.ISO_INSTANT.format(Instant.ofEpochSecond(timestampSunrise)).substring(11, 16)
        ...
        sunset.text = DateTimeFormatter.ISO_INSTANT.format(Instant.ofEpochSecond(timestampSunset)).substring(11, 16)
